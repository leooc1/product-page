...
export default function Add() {
    ...
    return (
        <div className='flex items-center sm:items-start flex-col sm:flex-row justify-center md:justify-normal'>
            <div className='bg-slate-100 flex sm:py-1 py-3 sm:px-0 px-2 justify-between font-bold text-lg h-fit items-center rounded-lg sm:mr-4'>
                <button className='text-[var(--secondary)] text-2xl h-7 w-6 flex justify-center items-center mr-4 sm:mr-0' onClick={() => {
                    if (num > 0) {
                        setNum(num - 1)
                    }
                }}><Image src='images/icon-minus.svg' alt='menos' width={10} height={10}/></button>
                <input className='w-14 text-center' type="number" name="quant" id="quant" value={num} disabled/>
                <button className='text-[var(--secondary)] text-2xl h-7 w-6 flex justify-center items-center ml-4 sm:ml-0' onClick={() => {
                    setNum(num + 1)
